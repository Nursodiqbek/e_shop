...
class RegisterApiView(APIView):
    ...
    def post(self, request):
        ...
        if CustomUser.objects.filter(email=email):
            return Response({'message': 'Bu email orqali oldin royxatdan otilgan!!'})

        # else:
        #     send_password = str(random_password.random_password(6))
        #     send_sms.send_email(send_email=email, message=send_password)
    ...
    def post(self, request):
        ...
        if user:
            ...
            send_sms.send_email(send_email=email,
                                message=f'Bizning tizimga kirish uchun {code}. bu kodni hech kimga bermang!')
            return Response({
                'code_token': code_token
            })
